...
var wormSize = 200;
var wormSizeDiv = document.getElementById("wormSizeValue");
updateWormSizeDisplay();

function updateWormSizeDisplay() {
  wormSizeDiv.textContent =
    wormSize - 200 + " (Max Age: " + (deadAt - 200) + ")";
}

var lastFoodTime = Date.now();
...
var chain = new IKChain(wormSize, 1);
...
function update() {
  ...
  for (var i = 0; i < food.length; i++) {
    if (
      ...
      if (
        ...
        food.splice(i, 1);
        chain.links.push(
          new IKSegment(1, chain.links[chain.links.length - 1].tail)
        );
        wormSize++;
        updateWormSizeDisplay();
      }
  ...
  // Update IK chain
  chain.update(target);
  if (Date.now() - lastFoodTime >= 60000 && food.length === 0) {
    generateFood();
...
function draw() {
  ...
  ctx.lineWidth = 20 + (wormSize - 200) / 10;
  ctx.lineJoin = "round";
  ctx.lineCap = "round";
...
function decreaseLineOpacity(opacity) {
...
if (wormSize == 225) {
  decreaseLineOpacity(0.5);
}

if (wormSize == 250) {
  decreaseLineOpacity(0.4);
}

if (wormSize == 275) {
  decreaseLineOpacity(0.3);
}

if (wormSize == 300) {
  decreaseLineOpacity(0.2);
}
...
setInterval(function () {
  if (alive) {
    ...
    ctx.fillStyle = "white";
    ctx.fillText(
      "The worm has died at age " + (wormSize - 200) + "!",
